...
&emsp;
>头文件 7.10_pose_estimation_3d3d.hpp
```c++
#ifndef POSE_ESTIMATION_3D3D_HPP 
#define POSE_ESTIMATION_3D3D_HPP 
#include <iostream>
...
&emsp;
>7.10_pose_estimation_3d3d.cpp
```c++
#include "7.10_pose_estimation_3d3d.hpp"
...
int main ( int argc, char** argv )
{
    ...
    // 建立3D点
    Mat depth1 = imread ( "./chapter7/1_depth.png", IMREAD_UNCHANGED );       // 深度图为16位无符号数，单通道图像
    Mat depth2 = imread ( "./chapter7/2_depth.png", IMREAD_UNCHANGED );       // 深度图为16位无符号数，单通道图像
...
```
`ICP` 的实现方式和前文讲述的是一致的。我们调用 $Eigen$ 进行 `SVD`，然后计算 $\pmb{R}, \pmb{t}$ 矩阵。我们输出了匹配后的结果，不过请注意，由于前面的推导是按照 $\pmb{p}_i = \pmb{R}\pmb{p}_i' + \pmb{t}$ 进行的，这里的 $\pmb{R}, \pmb{t}$ 是第二帧到第一帧的变换，与前面 `PnP` 部分是相反的。所以在输出结果中，我们同时打印了逆变换。
